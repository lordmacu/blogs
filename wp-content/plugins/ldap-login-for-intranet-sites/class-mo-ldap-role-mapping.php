...
class Mo_Ldap_Local_Role_Mapping{
	...
	function test_configuration($username){
		if(!Mo_Ldap_Local_Util::is_extension_installed('mcrypt')) {
			...
			$memberOfAtrributesArray = array(); //$this->get_member_of_attribute($username,"");
			...
			if($flag==0){
						...
						if(get_option('mo_ldap_local_mapping_value_default')) 
							echo "<span style=color:green;font-size:24px><b>Test Successful. </b></span><br><br> Default Role <b>".get_option('mo_ldap_local_mapping_value_default')."</b> will be assigned to the User. ";
						else
							echo "<span style=color:red;font-size:24px><b>Default Role Not Configured</b></span><br><br> Please configure and save default role for the user first. ";
			}
			echo "</div>";
