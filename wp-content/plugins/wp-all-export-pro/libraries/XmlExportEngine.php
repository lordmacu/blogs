		...
		protected function init(){
			...
			if ('advanced' == $this->post['export_type']) { 
				...
				if( "" == $this->post['wp_query'] ){
					...
					{
						self::$exportQuery = eval('return new WP_User_Query(array(' . $this->post['wp_query'] . ', \'orderby\' => \'ID\', \'order\' => \'ASC\', \'offset\' => 0, \'number\' => 10));');
						if (empty(self::$exportQuery->results)){
							$this->errors->add('form-validation', __('No matching users found', 'pmxe_plugin'));	
						}
						else{					
