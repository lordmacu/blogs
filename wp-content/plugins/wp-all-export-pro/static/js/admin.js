...
(function($){$(function () {
	...
	var init_filtering_fields = function(){
	    ...
	    $('.wp_all_export_filtering_rules').pmxe_nestedSortable({
	        ...
	        update: function () {	        
	        	$('.wp_all_export_filtering_rules').find('.condition').show();
	        	$('.wp_all_export_filtering_rules').find('.condition:last').hide(); 
	        	liveFiltering();    								
		    }
	    });
	...
	var liveFiltering = function(){
		...
		$.ajax({
			...
			success: function(response) {	
				...
				$('.wpallexport-choose-file').find('.wpallexport-filtering-wrapper').slideDown();
	...
	$('.wp_all_export_filtering_rules').find('.remove-ico').live('click', function(){
		...
		{
			$('.wp_all_export_filtering_rules').find('li:last').find('.condition').hide();
		}		
		liveFiltering();
	});
